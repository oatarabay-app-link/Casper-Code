...
const Dashboard = () => {
  ...
  const { fetchServers , serverList } = useServerStore();
  ...
  return (
      <div className="min-h-screen bg-gray-900">
        {/* Header */}
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4 pt-6">
        <div>
              ...
              <tbody>
                {serverList.map((server, i) => (
                  <tr key={i} className="border-t border-gray-700 hover:bg-gray-750 transition-colors duration-200">
                    <td className="py-3 px-3 text-white">{server.serverName}</td>
                    <td className="py-3 px-3 text-gray-300">{server.location}</td>
                    <td className="py-3 px-3 font-mono text-gray-300">{server.ipAddress}</td>
                    <td className="py-3 px-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                        server.serverStatus === 'Online' ? 'bg-green-900 text-green-300 border border-green-700' :
                        server.serverStatus === 'Maintenance' ? 'bg-yellow-900 text-yellow-300 border border-yellow-700' :
                        'bg-red-900 text-red-300 border border-red-700'
                      }`}>
                        {server.serverStatus}
                      </span>
                    </td>
                    <td className="py-3 px-3 text-gray-300">{server.users}</td>
                    <td className="py-3 px-3 text-gray-300">{server.load}</td>
                    <td className="py-3 px-3 text-gray-300">{server.uptime}</td>
