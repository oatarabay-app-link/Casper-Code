...
const Database = () => {
  const stats = useDatabaseStore((state) => state.stats);
  const status = useDatabaseStore((state) => state.status);
  const performance = useDatabaseStore((state) => state.performance);
  const backups = useDatabaseStore((state) => state.backups);
  const queries = useDatabaseStore((state) => state.queries);
  const refreshStats = useDatabaseStore((state) => state.refreshStats);
  const queueBackup = useDatabaseStore((state) => state.queueBackup);
  const isBackupQueued = useDatabaseStore((state) => state.isBackupQueued);

  const backupButtonClasses = isBackupQueued
    ? 'bg-yellow-900 border border-yellow-700 text-yellow-200 cursor-not-allowed'
      ...
      <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={() => queueBackup()}
            disabled={isBackupQueued}
            className={`flex items-center gap-2 px-4 py-2 text-sm rounded-lg transition-colors duration-200 ${backupButtonClasses}`}
          >
            <Backup fontSize="small" />
            {isBackupQueued ? 'Queued…' : 'Create Backup'}
          </button>
        </div>
      </div>
      ...
      <Section title="Recent Backups">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-300">
            ...
            <tbody>
              {backups.map((b, i) => (
                <tr key={b.id ?? i} className="border-t border-gray-700 hover:bg-gray-750 transition-colors duration-200">
                  <td className="px-4 py-3 text-white">{b.type}</td>
                  <td className="px-4 py-3 text-gray-300">{b.time}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-3 py-1 rounded-full ${getBackupStatusColor(b.status)}`}>
                      {b.status}
                    </span>
                  </td>
                </tr>
