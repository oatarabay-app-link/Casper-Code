...
namespace Casper.API.Services
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<Result<RoleCreateResponse>> CreateRoleAsync(string roleName)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (roleName.Length < 2)
                        validationErrors.Add("Role name must be at least 2 characters long");
                    if (roleName.Length > 50)
                        validationErrors.Add("Role name cannot exceed 50 characters");
                    if (!System.Text.RegularExpressions.Regex.IsMatch(roleName, @"^[a-zA-Z0-9_\s]+$"))
                        validationErrors.Add("Role name can only contain letters, numbers, spaces, and underscores");
                }

                if (validationErrors.Any())
                {
                    _logger.LogWarning("Validation failed for role creation: {Errors}", string.Join("; ", validationErrors));
                    return Result<RoleCreateResponse>.ValidationFailure(validationErrors);
                }
                ...
                if (await _roleManager.RoleExistsAsync(roleName))
                {
                    ...
                    return Result<RoleCreateResponse>.Conflict($"Role '{roleName}' already exists");
                }
                ...
                if (!result.Succeeded)
                {
                    ...
                    _logger.LogWarning("Failed to create role {RoleName}. Errors: {Errors}", roleName, string.Join("; ", errors));
            ...
            {
                _logger.LogError(dbEx, "Database error creating role: {RoleName}. Inner: {InnerException}",
                    roleName, dbEx.InnerException?.Message);
                return Result<RoleCreateResponse>.Failure("A database error occurred while creating the role", ErrorType.InternalError);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error creating role: {RoleName}. Error: {ErrorMessage}",
                    roleName, ex.Message);
                return Result<RoleCreateResponse>.Failure($"An unexpected error occurred: {ex.Message}", ErrorType.InternalError);
            }
        }
        ...
        public async Task<Result<RoleWithDetails>> CreateRoleWithPermissionsAsync(CreateRoleRequest request)
        {
            try
            {
                ...
                if (request == null)
                    return Result<RoleWithDetails>.Failure("Request body is required", ErrorType.Validation);
                ...
                else
                {
                    ...
                    if (request.RoleName.Length < 2)
                        validationErrors.Add("Role name must be at least 2 characters long");
                    if (request.RoleName.Length > 50)
                        validationErrors.Add("Role name cannot exceed 50 characters");
                    if (!System.Text.RegularExpressions.Regex.IsMatch(request.RoleName, @"^[a-zA-Z0-9_\s]+$"))
                        validationErrors.Add("Role name can only contain letters, numbers, spaces, and underscores");
                }
                ...
                if (request.Permissions != null)
                {
                    foreach (var permission in request.Permissions)
                    {
                        ...
                        {
                            validationErrors.Add("Permission names cannot be null or empty");
                            break;
                        }
                    }
                }
                ...
                if (validationErrors.Any())
                {
                    _logger.LogWarning("Validation failed for role creation with permissions: {Errors}",
                        string.Join("; ", validationErrors));
                    return Result<RoleWithDetails>.ValidationFailure(validationErrors);
                }
                ...
                if (await _roleManager.RoleExistsAsync(request.RoleName))
                {
                    ...
                    return Result<RoleWithDetails>.Conflict($"Role '{request.RoleName}' already exists");
                }
                ...
                if (request.Permissions.Any())
                {
                    ...
                    {
                        _logger.LogWarning("Invalid permissions specified: {Permissions}",
                            string.Join(", ", invalidPermissions));
                        return Result<RoleWithDetails>.Failure(
                            $"The following permissions do not exist or are inactive: {string.Join(", ", invalidPermissions)}",
                ...
                var assignedPermissions = new List<string>();
                ...
                if (request.Permissions.Any())
                {
                    foreach (var permissionName in request.Permissions)
                    {
                        ...
                        {
                            failedPermissions.Add(permissionName);
                    ...
                    // Log permission assignment summary
                    if (failedPermissions.Any())
                    {
                        _logger.LogWarning("Role {RoleName} created but failed to assign {FailedCount} permissions: {Permissions}",
                            request.RoleName, failedPermissions.Count, string.Join(", ", failedPermissions));
                    }
                    else
                    {
                        _logger.LogInformation("All {PermissionCount} permissions assigned successfully to role {RoleName}",
                            assignedPermissions.Count, request.RoleName);
                    }
                }
                ...
                if (!string.IsNullOrWhiteSpace(request.Description))
                {
                    try
                    {
                        await _roleManager.AddClaimAsync(role, new Claim("Description", request.Description.Trim()));
                        _logger.LogInformation("Description claim added to role {RoleName}", request.RoleName);
                    }
                    catch (Exception claimEx)
                    {
                        _logger.LogWarning(claimEx, "Failed to add description claim to role {RoleName}", request.RoleName);
                        // Continue even if claim fails - it's not critical
                    }
                }
                ...
                _logger.LogInformation("Role created successfully with permissions: {RoleName}. Assigned: {AssignedCount}/{TotalCount}",
                    request.RoleName, assignedPermissions.Count, request.Permissions.Count());

                return Result<RoleWithDetails>.Success(roleDetails);
            }
            catch (DbUpdateException dbEx)
            {
                _logger.LogError(dbEx, "Database error creating role with permissions: {RoleName}. Inner: {InnerException}",
                    request.RoleName, dbEx.InnerException?.Message);

                if (dbEx.InnerException?.Message.Contains("foreign key constraint") == true)
                {
                    return Result<RoleWithDetails>.Failure("Referenced permission does not exist", ErrorType.Validation);
                }

                return Result<RoleWithDetails>.Failure("A database error occurred while creating the role", ErrorType.InternalError);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error creating role with permissions: {RoleName}. Error: {ErrorMessage}",
                    request.RoleName, ex.Message);
                return Result<RoleWithDetails>.Failure($"An unexpected error occurred: {ex.Message}", ErrorType.InternalError);
            }
        }
