        ...
        public async Task<Result<dynamic>> CreateDiscountAsync(CreateDiscountRequest request)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.Name))
                    validationErrors.Add("Discount name is required");

                if (string.IsNullOrWhiteSpace(request.Plan))
                    validationErrors.Add("Plan is required");
                ...
                var existingDiscount = await _context.PlanDiscounts
                    .FirstOrDefaultAsync(d => d.Name.ToLower() == request.Name.ToLower() &&
                                            d.Plan.ToLower() == request.Plan.ToLower() &&
                                            d.IsActive);
            ...
            catch (Exception ex)
            {
                ...
                return Result<dynamic>.Failure("Error creating discount", ErrorType.InternalError);
            }
        }
