...
@section("inner_content")
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ url('/admin/auth/user/'. $user->id .'/payments/-logs' . $payment->id) }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ method_field('PATCH') }}
                            {{ csrf_field() }}

                            @include ('backend.auth.user.payments.form', ['formMode' => 'edit'])
