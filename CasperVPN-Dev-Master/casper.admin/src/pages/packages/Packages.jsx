...
const textFromFeatureMatrix = (matrix = []) =>
  (Array.isArray(matrix) ? matrix : [])
    .map((m) => `${m.label} | ${m.detail ?? ""}`)
    .join("\n");

const textFromProtocols = (protocols = []) =>
(Array.isArray(protocols) ? protocols : []).join(", ");
...
const Packages = () => {
  ...
  const openEditModal = (plan) => {
    ...
    setForm({
      ...
      features: plan.featureMatrix.map((item) => `${item.label} | ${item.detail}`).join('\n'),
 ...
 try {
      if (mode === "edit") {
        await updatePlan({ id: form.id, ...form });
      } else {
        await addPlan({ ...form });
      }
      closeModal();
    } catch (err) {
      // Service throws validation errors with err.details, or server errors in response
      if (err?.details) {
        setServerErrors(err.details);
      } else if (err?.response?.data) {
        setServerErrors(err.response.data);
      } else {
        setServerErrors({ general: err.message || "Unknown error" });
      }
      // keep modal open so user can see messages
    }
  };
