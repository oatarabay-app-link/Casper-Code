...
import de.blinkt.openvpn.activities.DisconnectVPNDialog;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public static String humanReadableByteCountLeft(long bytes, boolean mbit) {
        if (mbit)
            bytes = bytes * 8;
        bytes = (500*8*1024*1024*1024) - bytes;
        int unit = mbit ? 1000 : 1024;
        if (bytes < unit)
            return bytes + (mbit ? " bit" : " B");
...
//        else
    ...
    private void jbNotificationExtras(int priority,
        ...
        try {
            ...
            Intent disconnectVPN = new Intent(this, DisconnectVPNDialog.class);
    ...
    synchronized void registerDeviceStateReceiver(OpenVPNManagement magnagement) {
        VpnStatus.logInfo("OpenVPN Service - registerDeviceStateReceiver");
    ...
    synchronized void unregisterDeviceStateReceiver() {
        if (mDeviceStateReceiver != null)
            ...
            VpnStatus.logInfo("OpenVPN Service - unregisterDeviceStateReceiver");
            try {
